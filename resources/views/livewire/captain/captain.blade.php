<x-slot name="title">{{__("Captain")}}</x-slot>
<x-slot name="header">{{__("Captain")}}</x-slot>

<div class="card card-custom">
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
        <div class="card-title">
            <h3 class="card-label">{{__("Captain")}}: {{ $captains->count() }}
    ...
    <div class="card-body table-responsive">
        <div class="row">
            ...
            <x-form.select wire:model.live="selectStatus" name="selectStatus" :label="__('Status')">
        ...
        <x-table.table>
            ...
            <x-slot name="body">
                ...
                @foreach($captains as $captain)
                    <x-table.row wire:key="{{ $captain->id }}" wire:loading.class="opacity-50">
                        ...
                        <x-table.cell>
                           @if($captain->register_step>1)
                                <x-buttons.icon :url="route('captains.edit', $captain->id)" wire:navigate/>
                            @else
