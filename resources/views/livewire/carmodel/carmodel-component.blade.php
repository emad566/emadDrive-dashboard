<x-slot name="title">{{__('Models')}}</x-slot>
<x-slot name="header">{{__('Models')}}</x-slot>
...
<div class="card card-custom" x-data="{
    ...
    <div class="card-header flex-wrap border-0 pt-6 pb-0">
        ...
        <div class="card-toolbar">
            <a wire:click.prevent="create" class="btn btn-primary font-weight-bolder" x-on:click="show=true; isCreate=true" >
                <i class="la la-plus"></i>{{__("Add")}} {{__("Model")}}
            </a>
        </div>
    </div>
    ...
    <div class="card-body table-responsive">
        ...
        <x-table.table>
            <x-slot name="head">
                ...
                <x-table.heading >{{ __('Brand') }}</x-table.heading>
            ...
            <x-slot name="body">
                @foreach($items as $item)
                    <x-table.row wire:loading.class="opacity-50" wire:target="search">
                        ...
                        <x-table.cell>{{ $item->brand->$langName }}</x-table.cell>
                        <x-table.cell>
    ...
    @include('livewire.carmodel.partials.create')
</div>
