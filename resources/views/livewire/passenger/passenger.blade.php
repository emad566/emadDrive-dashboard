<x-slot name="title">{{ __('Passengers') }}</x-slot>
<x-slot name="header">{{  __('Passengers')  }}</x-slot>
            ...
            <x-slot name="body">
                ...
                @foreach($passengers as $passenger)
                    <x-table.row wire:loading.class="opacity-50">
                        ...
                        <x-table.cell>
                            <x-buttons.delete actionId="{{ $passenger  ->id }}" x-on:click="sweetDelete('{{ $passenger  ->id }}')" />
