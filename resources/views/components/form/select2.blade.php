...
<div id="{{ $id }}" class="form-group {{ $id }} {{ $wrapperClasses }} ">
    ...
    <select  id="{{ $inputId }}" {{ $attributes }} name="{{ $name }}" class="form-control px-10 @if($select2) select2 @endif {{ $class }} @if($errors->first($name)) outline outline-red-800 outline-1 @endif" >
        {{ $slot }}
    </select>
